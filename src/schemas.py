...
class TriageResult(BaseModel):
    ...
    intent: str = Field(
        default="",
        description="The intent of the case what the customer is trying to achieve",
    )
    customer_id: str | None = Field(
        default=None,
        description="The customer id of the case",
    )
    account_id: str | None = Field(
        None,
        description="The account id of the case, if applicable",
    )
    ...
    amount: float | None = Field(
        None,
        description="The amount of the case, if applicable",
    )
    ...
    reasoning: str = Field(
        default="",
        description="The reasoning of the model for the triage result",
    )
...
class ProposedAction(BaseModel):
    action_type: ActionType = Field(
        ...,
        description="The type of action that the model is proposing to take",
    )
    description: str = Field(
        default="",
        description="The description of the action that the model is proposing to take",
    )
    amount: float | None = Field(
        default=None,
        description="The amount of the action that the model is proposing to take, if applicable",
    )
    citation: str = Field(
        default="",
...
class Draft(BaseModel):
    ...
    action: ProposedAction | None = Field(
        default=None,
        description="The action the specialist proposes, if any",
    )
    ...
    notes: str = Field(
        default="",
        description="Internal notes for the agent to consider when reviewing the draft. "
...
class Critique(BaseModel):
    ...
    grounded: bool = Field(
        default=False,
        description="True if the claims trace to retrive sources, "
        "False if the claims are not grounded in the retrieval",
    )
    problems: List[str] = Field(
        default_factory=list,
        description="The problems that the critic agent found in the draft",
    )
    fixes: List[str] = Field(
        default_factory=list,
        description="The fixes that the critic agent suggests for the draft",
    )
    escalation_reason: str | None = Field(
        default=None,
        description="The reason for escalation if the verdict is escalate",
    )
    rationale: str = Field(
        default="",
        description="The rationale of the critic agent for the verdict",
    )
...
class Source(BaseModel):
    doc_id: str = Field(
        ...,
        description="The document id from retrieval that the model is citing to support it's reply",
    )
    title: str = Field(
        default="",
        description="The title of the document that the model is citing to support it's reply",
    )
    domain: str = Field(
        default="",
        description="The domain of the document that the model is citing to support it's reply",
    )
    doc_type: str = Field(
        default="policy",
        description="The type of the document that the model is citing to support it's reply",
    )
    content: str = Field(
        default="",
        description="The content of the document that the model is citing to support it's reply",
    )
    score: float | None = Field(
        default=None,
        description="The score of the document that the model is citing to support it's reply",
    )
...
class ToolCall(BaseModel):
    ...
    args: dict = Field(
        ...
        description="The arguments to the tool that the model is calling",
    )
    ok: bool = Field(
        default=True,
        description="True if the tool call was successful, False otherwise",
    )
    result: str | None = Field(
        default=None,
        description="The result of the tool call. This will be filled"
        " after the tool is called.",
    )
    error: str | None = Field(
        default=None,
        description="The error message if the tool call was not successful. "
...
class TraceEvent(BaseModel):
    ...
    detail: str = Field(
        ...,
        description="The reasoning of the model for the action it is taking in the current step",
    )
